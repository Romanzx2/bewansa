...
async function saveTime(initials) {
    ...
    await addScore(initials, totalMilliseconds);
...
async function updateLeaderboard() {
    ...
    leaderboard.forEach((entry, index) => {
        ...
        if (isAdmin) {
            ...
            listItem.addEventListener('click', () => removeScore(entry.id));
        }
...
async function addScore(initials, totalMilliseconds) {
    await fetch('/api/updateLeaderboard', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ initials, totalMilliseconds }),
    });
}

document.addEventListener('DOMContentLoaded', () => {
    updateDisplay();
    getLeaderboard(); // Load and display the leaderboard from Firebase
});
